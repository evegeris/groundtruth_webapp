...
/*
      if (mouseCurX % 256 != 0){
        console.log('beforeX: '+mouseCurX.toString());
        mouseCurX = 256*(Math.floor(mouseCurX/256) + 1);
        console.log('afterX: '+mouseCurX.toString());
      }

      if (mouseCurY % 256 != 0){
        console.log('beforeY: '+mouseCurY.toString());
        mouseCurX = 256*(Math.floor(mouseCurY/256) + 1);
        console.log('afterY: '+mouseCurY.toString());
      }
      */
...
// Elaine edits
      if (size.w % 256 != 0){
      size.w = 256*Math.floor(size.w/256);
      this.setCenterPoint({x: mouseCurX - this._posDragStartX,
                           y: mouseCurY - this._posDragStartY});
      if(size.w > 256*4){size.w = 256*4;}
      if(size.w < 0){size.w = 256;}
      console.log("size.w: "+ size.w.toString());
      }
      else {
      console.log("else size.w: "+ size.w.toString());
      }

      if (size.h% 256 != 0){
      size.h = 256*Math.floor(size.h/256);
      this.setCenterPoint({x: mouseCurX - this._posDragStartX,
                           y: mouseCurY - this._posDragStartY});
      if(size.h > 256*4){size.h = 256*4;}
      if(size.h < 0){size.h = 256;}
      console.log("size.h: "+ size.h.toString());

      }
      else {
      console.log("else size.h: "+ size.h.toString());
      }
      */
    }
...
/*
// need imgs 256x256 for our purposes
console.log("s.w: "+String(s.w));
console.log("s.h: "+String(s.h));
console.log("s.w % 256 =  "+String(s.w % 256));

console.log("s.h % 256 =  "+String(s.h % 256));

if (s.w % 256 > 125){
  s.w = 256*(int(s.w/256) + 1);
  console.log("s.w: "+String(s.w));
}
else {
  s.w = 256*(int(s.w/256));
...
if (s.w % 256 != 0){
s.w = 256*Math.floor(s.w/256);
console.log("s.h: "+String(s.w));
}
else {
console.log("s.w: "+String(s.w));
}
    ...
    this.getResultImage=function() {
      ...
      if(image!==null){
        ...
        if (image.width % 256 != 0){
          console.log('beforeX: '+image.width.toString());
          image.width = 256*(Math.floor(image.width/256) + 1);
          console.log('afterX: '+image.width.toString());
        }

        if (image.height % 256 != 0){
          console.log('beforeY: '+image.height.toString());
          image.height = 256*(Math.floor(image.height/256) + 1);
          console.log('afterY: '+image.height.toString());
        }
        */
